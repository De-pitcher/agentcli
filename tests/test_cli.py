...
from agentcli.cli import run_chat, run_config, main
...
async def test_run_chat_preload_survives_trimming(monkeypatch, tmp_path):
    ...
    # After "hello again" (history length 4: sys + user + assistant + user). history_turns=1, so max messages = 2*1+1 = 3 + 1 (sys) = 4
    # Wait, history_turns=1 means max 2 prior messages (1 user, 1 asst) + 1 new user message = 3. Plus system = 4.
